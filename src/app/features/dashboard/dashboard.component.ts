...
export class DashboardComponent implements OnInit {
  private readonly api=inject(TitleApiService); readonly data=signal<TitleDashboard|null>(null); readonly error=signal('');
  readonly icons={ArrowRight,BookOpen,CircleAlert,FileCheck2,FileUp,Sparkles,TrendingUp};
  ngOnInit(){this.api.getDashboard().subscribe({next:value=>this.data.set(value),error:()=>this.error.set('Dashboard data could not be loaded. Please make sure the API is running.')});}
}
