...
export class TitleListComponent implements OnInit{
 ...
 readonly loading=signal(true);readonly records=signal<TitleRecord[]>([]);readonly total=signal(0);readonly selected=signal(new Set<number>());readonly filterOpen=signal(true);readonly toast=signal('');
 ...
 ngOnInit(){this.load();}
 load(){this.loading.set(true);this.api.search(this.filter).subscribe(result=>{this.records.set(result.items);this.total.set(result.totalCount);this.loading.set(false);});}
 ...
 remove(){if(!this.selected().size)return;this.api.deleteMany([...this.selected()]).subscribe({next:()=>{this.selected.set(new Set());this.notify('Selected titles deleted');this.load();},error:()=>this.notify('Demo mode: delete action previewed')});}
